...
teamRouter.get("/:id", async (req: any, res: any) => {
	try {
...
teamRouter.post("/", validateTeam, async (req: any, res: any) => {
...
teamRouter.put("/:id", validateTeam, async (req: any, res: any) => {
	const errors = validationResult(req);
	if (!errors.isEmpty()) {
		return res.status(400).json({ errors: errors.array() });
	}
...
teamRouter.delete("/:id", async (req: any, res: any) => {
	try {
		const teamId = Number(req.params.id);
